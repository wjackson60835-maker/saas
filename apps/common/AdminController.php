...
class AdminController extends Controller
{
    ...
    private function checkLevel()
    {
        ...
        // 侧栏「开奖预测区块」与私彩、数据采集同属开奖管理：有其一即可进入，避免仅写库菜单未同步 ay_role_level 时无法管理前台 yuce 条
        if (C === 'YuceBlock' && $levals !== array()) {
            foreach (array(
                '/admin/Collect/index',
                '/admin/Member/sicai'
            ) as $peer) {
                if (in_array($peer, $levals)) {
                    return true;
                }
            }
        }

        // 内容管理：有任一 /admin/Content/ 下菜单权限则允许 add/mod/del（菜单 URL 可能与 index 不完全一致、大小写亦兼容）
        if (strcasecmp(C, 'Content') === 0 && in_array(F, array(
            'add',
            'mod',
            'del'
        ), true)) {
            foreach ($levals as $lv) {
                if (! is_string($lv)) {
                    continue;
                }
                if (stripos($lv, '/admin/Content/') === 0) {
                    return true;
                }
            }
        }
