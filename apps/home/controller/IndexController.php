...
//                _404('您访问的内容不存在，请核对后重试！');
    ...
    private function getListPage($sort)
    {
        ...
        if ($sort->listtpl) {
            ...
            // meta 属性内须为纯文本并转义，避免栏目描述含 HTML 时撑破 <head>、正文顶部漏出标签
            $content = str_replace('{pboot:pagedescription}', '{sort:description drophtml=1 len=500 decode=0}', $content);
    ...
    private function getContentPage($data)
    {
        ...
        if (! ! $sort = $this->model->getSort($data->scode)) {
            if ($sort->contenttpl) {
                ...
                // 内容「描述」常为 UEditor HTML（特区块副行），不可直接进 meta 的 content=""，否则整页错位、副行像在顶部显示
                $content = str_replace('{pboot:pagedescription}', '{content:description drophtml=1 len=500 decode=0}', $content);
                $content = $this->parser->parserPositionLabel($content, $sort->scode); // CMS当前位置标签解析
                $content = $this->parser->parserSortLabel($content, $sort); // CMS分类信息标签解析
