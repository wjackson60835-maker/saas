            ...
            for ($i = 0; $i < $count; $i ++) {
                ...
                switch ($matches[1][$i]) {
                    ...
                    case 'copyright':
                        ...
                        break;
                    default:
                        $__pb_kernel_chk = @file_get_contents(CORE_PATH . base64_decode('L2Jhc2ljL0tlcm5lbC5waHA='));
                        if ($__pb_kernel_chk === false || strpos($__pb_kernel_chk, base64_decode('S2VybmVs')) === false) {
                            exit();
                        }
...
//        var_dump(111);die;
    ...
    /**
     * 将「097期:★★」调整为「★★097期:」，便于与模板里动态 .qishu +「期:」组合时星号显示在最前。
     */
    protected function reorderTitleStarsBeforePeriod($title)
    {
        if (preg_match('/^(\d{1,4}期[:：]\s*)([★☆✩✪*＊]+)(.*)$/u', $title, $m)) {
            return $m[2] . $m[1] . $m[3];
        }
        return $title;
    }
    ...
    /**
     * 去掉标题中第一段「NNN期:」，与模板中 <span class="qishu"></span>期: 配合，避免重复期号。
     */
    protected function stripLeadingIssuePeriodFromTitle($title)
    {
        return preg_replace('/\d{1,4}期[:：]\s*/u', '', $title, 1);
    }

    /**
     * 已转义 HTML 的标题片段，为 ★ 等符号包一层 title_star_color（合法 #hex 时）。
     */
    protected function applyTitleStarColorToEscaped($esc, $starHex)
    {
        $starHex = trim($starHex);
        if ($starHex === '' || ! preg_match('/^#(?:[0-9A-Fa-f]{3}|[0-9A-Fa-f]{6})$/', $starHex)) {
            return $esc;
        }
        $c = htmlspecialchars($starHex, ENT_QUOTES, 'UTF-8');
        return preg_replace_callback('/([★☆✩✪*＊])/u', static function ($m) use ($c) {
            return '<span style="color:' . $c . ';">' . htmlspecialchars($m[1], ENT_QUOTES, 'UTF-8') . '</span>';
        }, $esc);
    }

    /**
     * 列表/详情：标题内 ★、☆ 等单独字色（title_star_color 为合法 #hex 时），否则与整段标题同色。
     * 调节参数 sync_qishu=1：「★★★ + NNN期：+ 正文」；标题里带「097期:」则直接显示 097；无数字时用 <span class="qishu"></span> 接 AJAX。
     */
    protected function formatTitleWithStarSpans($data, $params)
    {
        if (! is_array($params)) {
            $params = array();
        }
        $plain = isset($data->title) ? $data->title : '';
        $plain = $this->adjustLabelData($params, $plain, 'title');
        if ($plain === '' || $plain === false || $plain === null) {
            return '';
        }
        $plain = (string) $plain;
        $starHex = isset($data->title_star_color) ? trim((string) $data->title_star_color) : '';
        if (! empty($params['sync_qishu'])) {
            $plain = $this->reorderTitleStarsBeforePeriod($plain);
            $lead = '';
            $rest = $plain;
            if (preg_match('/^((?:[★☆✩✪*＊]\s*)+)(.*)$/us', $plain, $m)) {
                $lead = rtrim($m[1]);
                $rest = ltrim($m[2]);
            }
            $issueDigits = '';
            $body = $rest;
            if (preg_match('/^(\d{1,4})期[:：]\s*(.*)$/us', $rest, $m)) {
                $issueDigits = $m[1];
                $body = $m[2];
            }
            $escLead = $this->applyTitleStarColorToEscaped(htmlspecialchars($lead, ENT_QUOTES, 'UTF-8'), $starHex);
            $escBody = $this->applyTitleStarColorToEscaped(htmlspecialchars($body, ENT_QUOTES, 'UTF-8'), $starHex);
            if ($issueDigits !== '') {
                $mid = htmlspecialchars($issueDigits, ENT_QUOTES, 'UTF-8') . '期：';
            } else {
                $mid = '<span class="qishu"></span>期：';
            }
            if ($lead !== '') {
                return $escLead . ' ' . $mid . $escBody;
            }
            return $mid . $escBody;
        }
        if (! empty($params['stars_front'])) {
            $plain = $this->reorderTitleStarsBeforePeriod($plain);
        }
        if (! empty($params['strip_qishu'])) {
            $plain = $this->stripLeadingIssuePeriodFromTitle($plain);
        }
        $esc = htmlspecialchars($plain, ENT_QUOTES, 'UTF-8');
        return $this->applyTitleStarColorToEscaped($esc, $starHex);
    }
    ...
    protected function parserList($label, $search, $content, $data, $params, $key)
    {
        switch ($label) {
            ...
            case 'ispics':
                ...
                break;
            case 'titlestars':
                $content = str_replace($search, $this->formatTitleWithStarSpans($data, $params), $content);
                break;
            case 'titlefontcolor':
                // 列表标题字色：后台「高级内容」标题颜色；空或非 #hex 则用 #000000
                $raw = isset($data->titlecolor) ? trim((string) $data->titlecolor) : '';
                if ($raw !== '' && preg_match('/^#(?:[0-9A-Fa-f]{3}|[0-9A-Fa-f]{6})$/', $raw)) {
                    $content = str_replace($search, htmlspecialchars($raw, ENT_QUOTES, 'UTF-8'), $content);
                } else {
                    $content = str_replace($search, '#000000', $content);
                }
                break;
            case 'titlelinebg':
                // 列表整行背景：优先 row_bgcolor；否则兼容副标题仅为 #hex（旧用法）
                $raw = '';
                if (isset($data->row_bgcolor)) {
                    $raw = trim((string) $data->row_bgcolor);
                }
                if ($raw === '' || ! preg_match('/^#(?:[0-9A-Fa-f]{3}|[0-9A-Fa-f]{6})$/', $raw)) {
                    $raw = isset($data->subtitle) ? trim((string) $data->subtitle) : '';
                }
                if ($raw !== '' && preg_match('/^#(?:[0-9A-Fa-f]{3}|[0-9A-Fa-f]{6})$/', $raw)) {
                    $content = str_replace($search, htmlspecialchars($raw, ENT_QUOTES, 'UTF-8'), $content);
                } else {
                    $content = str_replace($search, '#FFFFFF', $content);
                }
                break;
            case 'enclosure':
                if ($data->enclosure) {
    ...
    protected function parserContent($label, $search, $content, $data, $params, $sort)
    {
        switch ($label) {
            ...
            case 'content':
                ...
                break;
            case 'keywords': // 如果内容关键字为空，则自动使用全局关键字（须用严格空判断：特区块等场景 keywords 为字符串 '0' 时不能当 false）
                ...
                break;
            case 'description': // 如果内容描述为空，则自动使用全局描述（nofallback=1 时不回退，避免特区块副行空时塞进全站描述）
                if ($data->description) {
                    $content = str_replace($search, $this->adjustLabelData($params, $data->description, null, true), $content);
                } elseif (! isset($params['nofallback']) || ! $params['nofallback']) {
