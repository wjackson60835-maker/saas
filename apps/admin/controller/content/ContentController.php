...
class ContentController extends Controller
{
    ...
    /**
     * 列表行背景 / 标题星号色：直接从 $_POST 取 #hex 再 escape_string（与 titlecolor 入库方式一致，避免中间层滤掉 #）。
     */
    private function postHexColorOrEmpty(string $field, string $invalidMsg): string
    {
    ...
    public function add()
    {
        // 必须用请求方法判断：部分环境下 $_POST 可能为空但仍是 POST（否则误走 GET 分支会 location 丢提交）
        if (strtoupper((string) ($_SERVER['REQUEST_METHOD'] ?? '')) === 'POST') {
            ...
            $row_bgcolor = $this->postHexColorOrEmpty('row_bgcolor', '列表行背景色须为 # 开头的 3 或 6 位色值，或留空。');
            $title_star_color = $this->postHexColorOrEmpty('title_star_color', '标题星号颜色须为 # 开头的 3 或 6 位色值，或留空。');
            ...
            $data = array(
                ...
                'title_star_color' => $title_star_color,
                'subtitle' => $subtitle,
                'filename' => $filename,
                'author' => $author,
                'source' => $source,
            ...
            // 新增表单在「列表」页内嵌，GET 直达 add 时回到对应模型列表，避免空白页或被误判 404
            if (! $mcode = get('mcode', 'var')) {
                error('传递的模型编码参数有误，请核对后重试！');
            }
            location(url('/admin/Content/index/mcode/' . $mcode));
        }
    }
