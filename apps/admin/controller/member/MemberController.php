...
class MemberController extends Controller
{
    ...
    public function sicai()
    {
        if ((! ! $id = get('id', 'int')) && $result = $this->model->sicaigetMember($id)) {
            $this->assign('more', true);
            $this->assign('fields', $this->model->getFields());
            $this->assign('kjdata', $result);
        } else {
            $this->assign('list', true);
            if (! ! ($field = get('field', 'var')) && ! ! ($keyword = get('keyword', 'vars'))) {
                $result = $this->model->sicaifindMember($field, $keyword);
            } else {
                $result = $this->model->sicaigetList();
            }
        ...
        $this->display('member/sicai.html');
    }
    // 会员增加
    public function add()
    {
	 ...
	 // 私彩数据添加
    public function sicaiadd()
    {
        if ($_POST) {
            // 获取数据（与 sicaimod 一致：time 存 Unix 时间戳，nyr 存可读时间供列表展示）
            $time = post('time');
            ...
            // 执行添加
            if ($this->model->sicaiaddMember($data)) {
                $this->log('新增数据成功！');
                if (! ! $backurl = get('backurl')) {
                    success('新增成功！', base64_decode($backurl));
                } else {
                    success('新增成功！', url('/admin/Member/sicai'));
                }
            } else {
                $this->log('新增数据失败！');
                error('新增失败！', - 1);
    ...
    public function del()
    {
        ...
        // 删除私彩
    public function sicaidel()
    {
        // 执行批量删除
        if ($_POST) {
            if (! ! $list = post('list')) {
                if ($this->model->sicaidelMemberList($list)) {
                    $this->log('批量删除数据成功！');
                    success('删除成功！', - 1);
                } else {
                    $this->log('批量删除数据失败！');
        ...
        if ($this->model->sicaidelMember($id)) {
            $this->log('删除数据' . $id . '成功！');
            success('删除成功！', - 1);
        } else {
            $this->log('删除数据' . $id . '失败！');
    ...
    public function sicaimod()
{
        ...
        if ($_POST) {
            ...
            // 执行添加
            if ($this->model->sicaimodMember($id, $data)) {
                $this->log('修改会员成功！');
                if (! ! $backurl = get('backurl')) {
                    success('修改成功！', base64_decode($backurl));
                } else {
                    success('修改成功！', url('/admin/Member/sicai'));
                }
            } else {
                $this->log('修改会员失败！');
                error('修改失败！', - 1);
            ...
            // 调取修改内容
            $this->assign('sicaimod', true);
            if (! $result = $this->model->sicaigetMember($id)) {
                error('编辑的内容已经不存在！', - 1);
            }
            // 会员等级
            $this->assign('kjdata', $result);
